...
readonly TALENTAI_CORRELATION_QUERY="$TALENTAI_REPOSITORY_ROOT/database/queries/Q021__verify_end_to_end_correlation.sql"
...
if [[ ! "$TALENTAI_EXTRACTION_ID" =~ ^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-5][0-9a-fA-F]{3}-[89aAbB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$ ]]; then
  echo 'Usage: ./scripts/verify-talentai-correlation.sh <phase3-extraction-uuid>' >&2
  exit 1
fi
