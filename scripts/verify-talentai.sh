...
readonly TALENTAI_ASSESSMENT_WORKFLOW_DIR="$TALENTAI_REPOSITORY_ROOT/workflows/resume-assessment"

cd "$TALENTAI_REPOSITORY_ROOT"

required_workflow_files=(
  "$TALENTAI_ASSESSMENT_WORKFLOW_DIR/TAI-01-resume-intake-extraction-v2.json"
  "$TALENTAI_ASSESSMENT_WORKFLOW_DIR/TAI-02-grade-guide-resolver-v1.json"
  "$TALENTAI_ASSESSMENT_WORKFLOW_DIR/TAI-03-evidence-scoring-grade-engine-v1.json"
  "$TALENTAI_ASSESSMENT_WORKFLOW_DIR/manifest.json"
)
...
required_operational_files=(
  ...
  "$TALENTAI_SCRIPT_DIR/test-resume-assessment-workflows.sh"
  ...
  "$TALENTAI_REPOSITORY_ROOT/workflows/technical-interview/manifest.json"
  "$TALENTAI_REPOSITORY_ROOT/workflows/technical-interview/TAI-04-candidate-interview-final-grade-v1.json"
  "$TALENTAI_REPOSITORY_ROOT/workflows/technical-interview/TAI-05-secure-interview-invitation-v1.json"
  "$TALENTAI_REPOSITORY_ROOT/workflows/technical-interview/sql/load-completed-phase3-handoff-by-correlation.sql"
  ...
  "$TALENTAI_REPOSITORY_ROOT/database/queries/Q021__verify_end_to_end_correlation.sql"
  ...
  "$TALENTAI_SCRIPT_DIR/build-talentai-mvp-package.sh"
  "$TALENTAI_SCRIPT_DIR/build-phase45-mvp-package.sh"
  "$TALENTAI_SCRIPT_DIR/verify-talentai-correlation.sh"
  "$TALENTAI_SCRIPT_DIR/verify-phase45-correlation.sh"
  "$TALENTAI_SCRIPT_DIR/verify-phase1.sh"
  "$TALENTAI_REPOSITORY_ROOT/docs/runbooks/talentai-end-to-end.md"
...
' "$TALENTAI_ASSESSMENT_WORKFLOW_DIR/manifest.json" >/dev/null
...
if jq -s -e '
  ...
  | length > 0
' "$TALENTAI_ASSESSMENT_WORKFLOW_DIR"/TAI-*.json >/dev/null; then
  echo 'A test or obsolete node remains in a committed workflow.' >&2
  exit 1
fi

if rg -q '"pinData"|"staticData"|"credentials"' "$TALENTAI_ASSESSMENT_WORKFLOW_DIR"; then
