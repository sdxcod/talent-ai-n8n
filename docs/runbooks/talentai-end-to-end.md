...
3. An authorized HR operator uses TAI-05 to issue a secure invitation for the
   completed assessment.
4. The candidate opens TAI-04 with the one-time `invitationToken`.
5. TAI-04 claims the invitation, loads the Phase 3 handoff, and persists the
...
The database must include migrations V001 through V011. Verification must end
...
```bash
TALENTAI_RC_VERSION='3.1.1-rc.1'
./scripts/build-talentai-mvp-package.sh "$TALENTAI_RC_VERSION"
TALENTAI_RC_PACKAGE="$PWD/exports/private/TalentAI-mvp-v${TALENTAI_RC_VERSION}.n8np"
...
The import must report five workflows as `created` or `updated`, both
...
## Issue the secure invitation

1. Sign in with an HR/operator account that can execute TAI-05.
2. Open the published `talentai-secure-interview-invitation` form.
3. Issue an invitation for the completed Phase 3 `extractionId`.
4. Keep the returned candidate URL private and do not record its token in
   release evidence.

## Run the technical interview

1. Open the candidate URL containing the one-time `invitationToken`.
2. Start the interview so TAI-04 atomically claims the invitation.
...
Open the same candidate URL again. The already-claimed token must be rejected
without creating another session, question set, answer or result. Run the
correlation command again and confirm that the aggregate counts are unchanged.

## Recovery checks

Supported recovery starts through TAI-05 after a retryable failed interview or
an orphaned Claim has satisfied its recovery grace period. HR revokes the old
invitation and issues a new one for the same extraction. The new TAI-04
execution resumes from the last valid persisted checkpoint:
...
- controlled rejection of reuse of the claimed invitation;
