...
(
    SELECT
        ...
        $2::UUID AS assessment_id,
        $3::UUID AS extraction_id
)
SELECT
    CASE
    ...
    CASE
        ...
        ELSE 'Completed Phase 3 assessment resolved from a claimed invitation.'
...
LEFT JOIN talentai.assessment_execution AS execution
       ON execution.request_id = requested.request_id
      AND execution.assessment_id = requested.assessment_id
      AND execution.extraction_id = requested.extraction_id
...
LEFT JOIN talentai.grade_assessment AS assessment
       ON assessment.id = requested.assessment_id
      AND assessment.request_id = requested.request_id
